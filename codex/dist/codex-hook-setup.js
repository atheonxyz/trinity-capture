...
export function expectedHookRows(pluginRoot) {
    const commandByEvent = (eventName) => `node "${pluginRoot}/dist/codex-hook.js" ${eventName}`;
    return [
...
export async function checkHooks(rpc, pluginRoot = installedPluginRoot()) {
    const snapshot = await rpc.hooksList();
    return evaluateHooks(snapshot, pluginRoot);
}
export async function approveHooks(fingerprint, rpc, pluginRoot = installedPluginRoot()) {
    const before = await checkHooks(rpc, pluginRoot);
    if (before.status === "blocked" || before.status === "ready")
        return before;
    if (before.fingerprint !== fingerprint)
        return { status: "blocked", reason: "Refusing to approve stale fingerprint; rerun check and approve the current fingerprint." };
    const trustedHashes = Object.fromEntries(before.approvalRequiredHooks.map((hook) => [hook.key, { trusted_hash: hook.currentHash }]));
    try {
    ...
    if (after.status !== "ready")
        return { status: "blocked", reason: "Native Codex config write completed, but hooks did not become ready on reread." };
    if (after.pluginId !== before.pluginId || before.hooks.some((hook, index) => hook.key !== after.hooks[index]?.key || hook.currentHash !== after.hooks[index]?.currentHash))
        return { status: "blocked", reason: "Native Codex hook definitions changed after approval write; rerun check." };
    return after;
}
function evaluateHooks(snapshot, pluginRoot) {
    if (snapshot.errors.length > 0)
        return { status: "blocked", reason: `Native Codex returned hook errors: ${snapshot.errors.join("; ")}` };
    ...
    if (hooks.length !== HOOK_EVENTS.length)
        return { status: "blocked", reason: `Expected 6 Trinity hooks from ${expectedSourcePath}, found ${hooks.length}.` };
    ...
    for (const row of expected) {
        ...
        if (!hook)
            return { status: "blocked", reason: `Missing Trinity hook for ${row.eventName}.` };
    ...
    if (approvalRequiredHooks.length === 0)
        return { status: "ready", pluginId, hooks: publicHooks };
    return {
...
function validateHook(hook, expected, pluginId) {
    if (hook.key !== `${pluginId}:hooks/hooks.json:${expected.eventKey}:0:0`)
        return `Hook ${hook.key} has unexpected key for ${expected.eventName}.`;
    if (hook.source !== "plugin")
        return `Hook ${hook.key} has unexpected source ${hook.source}.`;
    if (hook.handlerType !== "command")
        return `Hook ${hook.key} is not a command hook.`;
    if (hook.command !== expected.command)
        return `Hook ${hook.key} has unexpected command.`;
    if (hook.matcher !== expected.matcher)
        return `Hook ${hook.key} has unexpected matcher.`;
    if (!hook.enabled)
        return `Hook ${hook.key} is disabled; enable the Trinity plugin hook in Codex before trusting it.`;
    if (hook.isManaged && hook.trustStatus !== "managed")
        return `Hook ${hook.key} has inconsistent managed hook state.`;
    if (hook.async)
        return `Hook ${hook.key} is unexpectedly async.`;
    return null;
}
function publicHook(hook) {
    if (hook.handlerType !== "command")
        throw new Error("internal error: non-command hook passed validation");
    return {
        key: hook.key,
