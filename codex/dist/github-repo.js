...
function loadCache(dataDir) {
    try {
        ...
        for (const [name, entry] of Object.entries(entries)) {
            ...
            const record = entry;
            const id = record.id;
            if ((typeof id !== "number" && id !== null) || typeof record.checkedAt !== "number")
                continue;
...
function parseRepositoryId(raw) {
...
function lookupWithGh(fullName, timeoutMs) {
    try {
...
async function lookupWithGitHubAPI(fullName, timeoutMs) {
    try {
...
export async function resolveGitHubRepository(dataDir, fullName, timeoutMs = 1_500) {
